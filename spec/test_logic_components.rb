...
describe "Beholder::LogicComponent" do
  ...
  describe "yield" do
    ...
    it "should yield the contents of the node" do
      strip_html(@view.parse('simple_yield_container')).should == "#{DOCTYPE}<html><head><title>Hello World</title></head><body><div>This text is inside the simple_yield_component</div></body></html>"
    end
    
    it "should correctly yield nested components" do
      strip_html(@view.parse('nested_yield_container')).should == "#{DOCTYPE}<html><head><title>Hello World</title></head><body><div><span>Message inside the nested_yield_component2: beholder2</span><span>Message inside the nested_yield_component1: beholder</span>Message inside the nested_yield_container: beholder</div></body></html>"
    end
    
    it "should provide access to the parent component's attributes for the yielded node" do
      strip_html(@view.parse('property_yield_container')).should == "#{DOCTYPE}<html><head><title>Hello World</title></head><body><div>Message inside the property_yield_component: beholder Message inside the property_yield_container: beholder</div></body></html>"
    end
  ...
  describe "if" do
    ...
    it "should replace the contents of the node if the value of 'condition' is true" do
      strip_html(@view.parse('simple_if')).should == "#{DOCTYPE}<html><head><title>Hello World</title></head><body>Body of 'if' component</body></html>"
    end
    
    it "should remove the node if the value of 'condition' is false" do
      strip_html(@view.parse('simple_if_false')).should == "#{DOCTYPE}<html><head><title>Hello World</title></head><body></body></html>"
    end
    
    it "should ignore component:elsif and component:else if the value of 'condition' is true" do
      strip_html(@view.parse('complex_if')).should == "#{DOCTYPE}<html><head><title>Hello World</title></head><body>Body of 'if' component</body></html>"
    end
  ...
  describe "elsif" do
    ...
    it "should replace the contents of the node if the value of 'condition' is true" do
      strip_html(@view.parse('complex_elsif')).should == "#{DOCTYPE}<html><head><title>Hello World</title></head><body>Body of 'elsif' component</body></html>"
    end
  end
  ...
  describe "else" do
    ...
    it "should replace the contents of the node if the value of 'condition' is false for component:if and all component:elsifs" do
      strip_html(@view.parse('else')).should == "#{DOCTYPE}<html><head><title>Hello World</title></head><body>Body of 'else' component</body></html>"
    end
